...
begin
  if (i>j) then busquedaDicotomica:=-1
    ...
    else if (v[med]>n) then busquedaDicotomica:=busquedaDicotomica(v,n,i,med-1)
    else busquedaDicotomica:=busquedaDicotomica(v,n,med+1,j);
  end;
end;
...
begin
     ...
     i:=1;
     j:=dimF;
     write('El numero esta en la posicion: ',busquedaDicotomica(v,num,i,j));
